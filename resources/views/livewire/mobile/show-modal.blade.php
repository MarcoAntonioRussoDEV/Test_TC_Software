<dialog id="show-modal" @class([
    ...
    @if ($task !== null)
        <div id="show-modal-box" class="modal-box cursor-default">
            ...
            <span class="py-4 text-sm">{{ $task->created_at }}</span>
            <div class="form-control mt-4">
                <p class="text-secondary">{{ $task->description }}</p>
                <div class="modal-action flex items-center justify-between">
                    <button class="btn btn-sm btn-ghost hover:bg-warning hover:text-warning-content" wire:click="edit">Edit</button>
                    <button type="submit" class="btn btn-sm hover:bg-error hover:text-error-content" wire:click="delete({{$task->id}})">Delete</button>
                </div>
            </div>
            @endif
        </dialog>
